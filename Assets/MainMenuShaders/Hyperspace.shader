Shader "MainMenu/Hyperspace"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // --- ordered dithering (matches the game look) ---
            ...
            void loadPalette(int sel, out float3 pal[8], out int n)
            {
                ...
                else { // 0 = Native (cold cyan wormhole)
                    pal[0]=float3(0.02,0.03,0.08); pal[1]=float3(0.02,0.10,0.22); pal[2]=float3(0.03,0.20,0.40); pal[3]=float3(0.0,0.42,0.62);
                    pal[4]=float3(0.05,0.62,0.82); pal[5]=float3(0.30,0.85,1.0);  pal[6]=float3(0.65,0.95,1.0); pal[7]=float3(0.92,0.99,1.0);
                    n=8; }
            }
            ...
            // Cyan wormhole. Mouse steers the centre; a click kicks a warp.
            float3 scene(float2 uv, float2 m, float2 clk, float clkT)
            {
                ...
                float warp  = exp(-clkT * 2.2) * _ClickScale;  // click impact scale
                float speed = 0.45 + 1.7 * warp;
                float z = iTime * speed + 0.32 / (r + 0.06);

                float rings  = 0.5 + 0.5 * sin(z * 3.0);
                float spokes = 0.5 + 0.5 * sin(a * 9.0 + z * 1.2);
                float bri  = 0.20 + 0.55 * pow(rings, 1.5) + 0.25 * spokes * rings;
                float core = smoothstep(0.5, 0.0, r);

                float t = bri * (0.35 + 0.9 * r) + core * 1.25;
                t += 0.06 * vnoise(float2(a * 5.0, z * 2.0));
                t = saturate(t);

                float3 col = ramp5(t,
                    float3(0.02, 0.03, 0.08), float3(0.03, 0.16, 0.34), float3(0.0, 0.5, 0.72),
                    float3(0.25, 0.85, 1.0),  float3(0.88, 0.98, 1.0));
                col += warp * 0.15 * spokes;
                return col;
            }
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                if (_Pixelation > 0.0001)
                {
                    ...
                    ditherCoord = cell;            // one dither sample per chunky pixel
                }

                float2 uv  = baseUV / _Scale;                                          // background scale

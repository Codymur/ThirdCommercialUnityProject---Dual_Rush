Shader "MainMenu/Steel"
{
    Properties
    {
        ...
        _DitherStrength ("Dither Strength",    Range(0,1)) = 0.13
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void loadPalette(int sel, out float3 pal[8], out int n)
            {
                ...
                else { // 0 = Native (cool silver / gunmetal)
                    pal[0]=float3(0.018,0.022,0.028); pal[1]=float3(0.07,0.085,0.10); pal[2]=float3(0.17,0.19,0.22);
                    pal[3]=float3(0.34,0.37,0.41);    pal[4]=float3(0.55,0.58,0.63);  pal[5]=float3(0.78,0.81,0.86);
                    pal[6]=float3(0.92,0.94,0.97);    n=7; }
            }
            float3 quantize(float3 col, float2 frag, float strength, float3 pal[8], int n)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    if (i < n) { float3 d = col - pal[i]; float dd = dot(d, d); if (dd < best) { best = dd; bc = pal[i]; } }
                }
                return bc;
            }

            // Minimalist brushed-steel sheen. A soft light glides with the mouse;
            // a click sends a single restrained ring. Mostly dark negative space.
            float steelField(float2 uv, float2 m, float2 clk, float clkT)
            {
                float2 p = uv;

                // broad diagonal base gradient
                float g = 0.5 + 0.5 * dot(normalize(float2(0.55, 1.0)), p);

                // slow large undulation so the surface feels alive, not static
                float wave = 0.5 + 0.5 * sin(p.x * 1.4 + p.y * 0.8 + iTime * 0.22);

                float base = 0.15 + 0.26 * g + 0.08 * wave;

                // specular highlight following the cursor
                float dM = length(uv - m);
                base += 0.50 * exp(-dM * 1.25);

                // a drifting brushed-light band tied loosely to the mouse height
                float by = (p.y - 0.18 * sin(iTime * 0.18) - m.y * 0.30) * 2.2;
                base += 0.12 * exp(-by * by);

                // restrained click ripple
                float rr = length(uv - clk) / _ClickScale;   // click impact scale
                base += 0.16 * sin(rr * 14.0 - clkT * 10.0) * exp(-rr * 2.6) * exp(-clkT * 2.0);

                base *= 1.0 - 0.28 * dot(uv, uv); // soft vignette
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float v = steelField(uv, m, clk, _ClickTime);
                // faint cool tint before quantizing to the palette
                float3 col = v * float3(0.97, 0.99, 1.03);

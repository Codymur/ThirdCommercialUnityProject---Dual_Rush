// DUAL RUSH — Perk Card Hover FX  ·  1/5  EMBER SPOTLIGHT
// A hot ember glow tracks the cursor across the card; edges ignite with a rim
// glow on hover. Retro dither posterizes the bloom. UI/Canvas compatible.
// Drive _Hover / _Mouse / _FxTime / _Aspect with PerkCardFX.cs.
Shader "DualRush/PerkCard/EmberSpotlight"
{
    Properties
    {
        ...
        _Glow ("Glow Strength", Range(0,3)) = 1.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i):SV_Target
            {
                ...
                float dist = length(d);
                float flick = 0.92 + 0.08 * sin(_FxTime * 22.0);
                float spot = smoothstep(0.62, 0.04, dist) * _Hover * _Glow * flick;
                spot = floor(spot * 5.0 + bd) / 5.0;           // dithered banding

                float edge = min(min(uv.x, 1.0 - uv.x), min(uv.y, 1.0 - uv.y));
                float rim = smoothstep(0.07, 0.0, edge) * _Hover;

                float3 col = _BaseColor.rgb;
                col += _AccentColor.rgb * spot;
                col += _AccentColor.rgb * rim * (0.6 + 0.4 * flick);
                col += (bd - 0.5) * 0.015;                     // faint idle grain

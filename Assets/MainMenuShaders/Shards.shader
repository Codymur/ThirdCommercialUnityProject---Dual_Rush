Shader "MainMenu/Shards"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 hash22(float2 p){ float n=sin(dot(p,float2(41.0,289.0))); return frac(float2(262144.0,32768.0)*n); }
            ...
            void loadPalette(int sel, out float3 pal[8], out int n)
            {
                ...
                else { // 0 = Native (synthwave Voronoi crystals)
                    pal[0]=float3(0.02,0.01,0.05); pal[1]=float3(0.10,0.02,0.18); pal[2]=float3(0.28,0.04,0.36); pal[3]=float3(0.55,0.08,0.52);
                    pal[4]=float3(0.85,0.16,0.62); pal[5]=float3(1.0,0.30,0.72);  pal[6]=float3(0.45,0.78,1.0);  pal[7]=float3(0.92,0.95,1.0);
                    n=8; }
            }
            ...
            // Drifting Voronoi crystals. Mouse lights nearby cells; a click
            // sends a shatter ring that flares the edges.
            float3 scene(float2 uv, float2 m, float2 clk, float clkT)
            {
                float2 p  = uv * 3.0;
                float2 ip = floor(p), fp = frac(p);
                float  f1 = 8.0, f2 = 8.0;
                float2 cid = float2(0.0, 0.0);
                ...
                for (int y = -1; y <= 1; y++)
                {
                    for (int x = -1; x <= 1; x++)
                    {
                        ...
                        o = 0.5 + 0.45 * sin(iTime * 0.6 + 6.2831 * o);
                        float2 r = g + o - fp;
                        float  d = dot(r, r);
                ...
                float eg    = smoothstep(0.09, 0.0, edge);
                float shade = hash21(cid);

                float dm   = length(uv - m);
                float prox = exp(-dm * dm * 3.0);
                float rr   = length(uv - clk) / _ClickScale;   // click impact scale
                float ring = sin(rr * 16.0 - clkT * 15.0) * exp(-rr * 1.8) * exp(-clkT * 1.5);

                float t = 0.12 + 0.45 * shade + prox * 0.5 + 0.35 * max(ring, 0.0);
                t = saturate(t);

                float3 col = ramp5(t,
                    float3(0.02, 0.01, 0.05), float3(0.20, 0.03, 0.30), float3(0.55, 0.08, 0.52),
                    float3(0.98, 0.25, 0.68), float3(1.0,  0.65, 0.92));
                col = lerp(col, float3(0.4, 0.85, 1.0), eg * (0.35 + 0.55 * prox));
                return col;
            }
